...
export const sendPasswordResetEmail = async (to: string, token: string) => {
  // Check if email is configured
  if (!config.email.host || !config.email.user || !config.email.pass) {
    logger.warn('Email service not fully configured. Skipping password reset email sending.', {
      host: config.email.host,
      user: config.email.user ? '***' : 'missing'
    });
    const resetUrl = `${process.env.FRONTEND_URL || 'http://localhost:3000'}/auth/reset-password?token=${token}`;
    logger.info('>>> MOCK EMAIL: Reset Password Link: ' + resetUrl);
    return;
  }
  ...
  const mailOptions = {
    from: config.email.from,
    to,
    subject: 'Nutrify - Reset Password',
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 10px;">
        <div style="text-align: center; margin-bottom: 20px;">
          <h1 style="color: #24B47E;">Nutrify</h1>
        ...
        <div style="text-align: center; margin: 30px 0;">
          <a href="${resetUrl}" style="display: inline-block; padding: 12px 24px; background-color: #EF4444; color: white; text-decoration: none; border-radius: 50px; font-weight: bold;">Reset Password</a>
        </div>
        <p style="color: #666;">Atau salin link ini ke browser:</p>
