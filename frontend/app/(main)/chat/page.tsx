    ...
    <div className="flex h-[calc(100vh-80px)] md:h-[calc(100vh-100px)] relative bg-gray-50/50 dark:bg-gray-900 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col h-full relative w-full min-w-0">
        ...
        <div className="h-16 px-4 md:px-6 flex items-center justify-between bg-white/80 dark:bg-gray-800/80 backdrop-blur-md border-b border-gray-100 dark:border-gray-700 z-30">
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              <div className="w-9 h-9 rounded-full bg-primary-action-50 dark:bg-primary-action-950/20 flex items-center justify-center text-primary-action">
                <svg viewBox="0 0 100 120" className="w-6 h-6" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M50 8C65 20 72 32 72 45C72 60 62 72 50 72C38 72 28 60 28 45C28 32 35 20 50 8Z" fill="#C4603A"/>
                  <ellipse cx="50" cy="52" rx="13" ry="17" fill="#FAF0E0" />
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 scroll-smooth pb-32">
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-center space-y-6 opacity-100 mt-[-50px] px-4">
              <div className="w-20 h-20 bg-primary-action-50 dark:bg-primary-action-950/20 border border-primary-action-100 dark:border-primary-action-900/50 rounded-2xl flex items-center justify-center shadow-warm-md">
                <svg viewBox="0 0 100 120" className="w-12 h-12 animate-float" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M50 8C65 20 72 32 72 45C72 60 62 72 50 72C38 72 28 60 28 45C28 32 35 20 50 8Z" fill="#C4603A"/>
                  <ellipse cx="50" cy="52" rx="13" ry="17" fill="#FAF0E0" />
                  <circle cx="50" cy="52" r="6" fill="#E8A838"/>
                  <rect x="44" y="71" width="12" height="16" rx="6" fill="#C4603A"/>
              ...
              <div className="max-w-md space-y-2">
                <h2 className="text-xl font-bold text-gray-900 dark:text-white font-display">Konsultasi Pola Makan Nusantara</h2>
                <p className="text-sm text-gray-500 dark:text-gray-400 font-light">
                  Tanyakan keselarasan gizi makanan daerah, saran menu mingguan, atau alternatif sehat masakan lokal Anda.
                </p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 w-full max-w-lg">
                {suggestions.map((s, i) => (
                  <button
                    key={i}
                    onClick={() => setInput(s)}
                    className="text-left text-xs p-4 rounded-xl bg-white dark:bg-gray-800 border border-border-warm dark:border-gray-700 hover:border-primary-action dark:hover:border-primary-action hover:shadow-warm-sm transition-all group"
                  >
                    <span className="text-gray-600 dark:text-gray-300 group-hover:text-primary-action dark:group-hover:text-primary-action transition-colors">{s}</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          {messages.map((m, idx) => (
            ...
            >
              {m.role === 'assistant' && (
                <div className="w-8 h-8 rounded-full bg-primary-action-50 dark:bg-primary-action-950/20 flex items-center justify-center text-primary-action flex-shrink-0 mt-1">
                  <svg viewBox="0 0 100 120" className="w-5 h-5" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M50 8C65 20 72 32 72 45C72 60 62 72 50 72C38 72 28 60 28 45C28 32 35 20 50 8Z" fill="#C4603A"/>
                    <ellipse cx="50" cy="52" rx="13" ry="17" fill="#FAF0E0" />
                    <circle cx="50" cy="52" r="6" fill="#E8A838"/>
                    <rect x="44" y="71" width="12" height="16" rx="6" fill="#C4603A"/>
              ...
              <div className={`max-w-[85%] md:max-w-[75%] rounded-2xl px-5 py-3.5 shadow-sm text-sm leading-relaxed ${m.role === 'user'
                ? 'bg-primary-action text-white rounded-br-none shadow-primary-action/10'
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-white via-white/90 to-transparent dark:from-gray-900 dark:via-gray-900/90">
          <div className="max-w-3xl mx-auto flex items-center gap-2 bg-white dark:bg-gray-800 p-2 pl-4 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700">
            ...
            <button
              onClick={onSend}
              disabled={!canSend}
              className="p-3 bg-primary-action hover:bg-primary-action/90 disabled:bg-gray-100 dark:disabled:bg-gray-700 disabled:text-gray-300 text-white rounded-xl transition-all shadow-lg shadow-primary-action/20 active:scale-95 flex-shrink-0"
            >
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="p-3">
        <button
          onClick={onNewChat}
          className="w-full flex items-center gap-2 justify-center py-2.5 px-4 bg-primary-action hover:bg-primary-action/90 text-white rounded-xl font-medium transition-colors shadow-primary-action/20 shadow-lg"
        >
        ...
        {history.length === 0 ? (
          ...
          history.map((conv: Conversation) => (
            <div
              ...
              className={`group w-full flex items-center justify-between p-3 rounded-xl text-sm transition-all cursor-pointer ${conversationId === conv.id ? 'bg-primary-action-50 dark:bg-primary-action-950/20 border-primary-action-200 dark:border-primary-action-800' : 'hover:bg-gray-50 dark:hover:bg-gray-700/50'} border border-transparent`}
            >
              <div className="min-w-0 flex-1 mr-2 text-left">
                <p className={`font-medium truncate ${conversationId === conv.id ? 'text-primary-action dark:text-primary-action-400 font-semibold' : 'text-gray-700 dark:text-gray-300'}`}>
