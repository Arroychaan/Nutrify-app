        ...
        <>
            ...
            <div className="max-w-3xl mx-auto space-y-6 pb-24 md:pb-8">
                ...
                <motion.div
                    ...
                    className="bg-primary-dark rounded-3xl p-6 text-white shadow-warm-lg border border-primary-dark-600 relative overflow-hidden"
                >
                    ...
                    <div className="flex items-center justify-between mb-5 relative z-10">
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-xl bg-primary-action/20 flex items-center justify-center text-primary-action">
                                <Flame className="w-5 h-5 fill-current" />
                            </div>
                            <span className="font-semibold text-neutral-300">{t('foodLog.totalCalories')}</span>
                        </div>
                        <span className="text-3xl font-bold tracking-tight font-display text-primary-action">{summary.calories} <span className="text-sm font-normal text-neutral-400">kkal</span></span>
                    </div>
                    <div className="grid grid-cols-3 gap-3 relative z-10">
                        <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-3 text-center border border-white/10">
                            <p className="text-[10px] uppercase tracking-wider text-neutral-400 mb-1 font-semibold">{t('dashboard.protein')}</p>
                            <p className="font-bold text-base text-white">{summary.protein}g</p>
                        </div>
                        <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-3 text-center border border-white/10">
                            <p className="text-[10px] uppercase tracking-wider text-neutral-400 mb-1 font-semibold">{t('dashboard.carbs')}</p>
                            <p className="font-bold text-base text-white">{summary.carbs}g</p>
                        </div>
                        <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-3 text-center border border-white/10">
                            <p className="text-[10px] uppercase tracking-wider text-neutral-400 mb-1 font-semibold">{t('dashboard.fat')}</p>
                            <p className="font-bold text-base text-white">{summary.fat}g</p>
                        </div>
                    </div>
                </motion.div>
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="space-y-2">
                                        {mealLogs.length === 0 ? (
                                            <div className="flex flex-col items-center justify-center py-10 text-center bg-surface/50 dark:bg-gray-800/30 rounded-2xl border border-dashed border-border-warm dark:border-neutral-700">
                                                <div className="w-12 h-12 mb-4 bg-background dark:bg-neutral-800 text-text-muted rounded-full flex items-center justify-center">
                                                    <Utensils className="w-5 h-5 text-primary-action" />
                                                </div>
                                                <p className="text-sm font-bold text-primary-dark dark:text-white">{t('foodLog.noLogs')} {getMealLabel(mealType).toLowerCase()}</p>
                                                <p className="text-xs text-text-muted mt-2">Ketuk + untuk mencatat makanan Anda</p>
                                            </div>
                                        ) : (
                                            mealLogs.map((log) => (
                                                <div
